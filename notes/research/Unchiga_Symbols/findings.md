...
| F72 | `0x8009B165` (1 byte) | Winner side for the F23 block: used as index into `g_DuelSavePtrs` (winner gets wins++ at ptr+0x518, loser = index^1 gets losses++ at ptr+0x51A). The CPU-win trace held 1 through the end-credit latch. The player-win control began after that prior loss, observed the stale 1 on duel entry, changed to 0 when the opponent's LP reached zero, and held 0 through a fresh latch. This confirms 0=player and 1=opponent. | `gDuel_bWinnerSide` | APPLIED |
| F73 | `0x80021E7C..0x80021FAC` (in `Duel_MaybeEndOfDuel` 0x800218F0) | One-shot end-of-duel credit block, latched by bit 0x2000 of the state half 0x8009B23A. Branch at 0x80021EA4: `D_8009B360 >= 0` OR `gDuel_bOpponentID < 0` → W/L record path (wins/losses u16, cap 0x270F, via `g_DuelSavePtrs` + 0x518/0x51A); else → starchips path: `gLibrary_dwStarchips += unk2E0->0x3A`, cap 0xF423F (999999), then the drop award `jal 0x80021894`. In a 1P Free Duel this path addresses ptr0+0x518 = 0x801D0718, which is separate from the per-duelist grid record updated by `FreeDuel_Init` in F75/F156. | (fact) | CONFIRMED |
