...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F10 | `SD_SetOutputType(mode)` | Fired on every stereo/mono toggle with a0=0 (STEREO) / a0=1 (MONO), both directions. Matched body: writes soundState(D_8009B45C)->f48, toggles flag 0x815, re-derives CD volume via func_80044DC0. Exact DotR vocabulary for this role. | `SD_SetOutputType` | APPLIED |
| F11 | `0x8009B408` (byte) | Flips 0<->1 with the setting (0=STEREO, 1=MONO), live, both directions, stable across double-checks. `Options_Init` derives `gOptions_bOutputType` from it (sign-bit = reset semantics), so this is the authoritative stored setting. | `gSD_bOutputType` | APPLIED |
...
| F32 | `gDuel_awRecentCardDrops` | On purchase the list SHIFTED with card 2 inserted at the front, proving that drops and password buys share one recently acquired list. The later exact-C and extent check in F61 established its capacity as 16 cards rather than the ten entries sampled here. | (semantic fix) | CONFIRMED |
...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F70 | `gFreeDuel_aDuelistRecords` (`0x801D071C`, 40 × 4 bytes) | Free Duel grid records: slot 0 is the Build Deck tile, while duelist IDs 1–39 contain u16 wins at +0 and u16 losses at +2. The Free Duel init increment clamps each counter at 999. Pinned live: a natural win vs Simon landed at 0x801D0720 while `gDuel_bOpponentID` read 1 on his results screen; writes to 0x724/0x726 showed on Teana's grid row (id 2). Cross-check: GameShark "DarkNite max wins" 0x801D07B0 = base + 37*4. WARNING: the community "Simon 0 losses" code (0x801D0724) is an off-by-one in THEIR db — that address is id 2's wins. | `gFreeDuel_aDuelistRecords` | APPLIED |
...
| F72 | `0x8009B165` (1 byte) | Winner side for the F23 block: used as index into `g_DuelSavePtrs` (winner gets wins++ at ptr+0x518, loser = index^1 gets losses++ at ptr+0x51A). The CPU-win trace held 1. The player-win control began after that prior loss, observed the stale 1 on duel entry, changed to 0 when the opponent's LP reached zero, and held 0 through the new result-fade marker in D_8009B23A. This confirms 0=player and 1=opponent in that single-player control, not direct observation of counter writes. The final 0xC6 sample is already the Free Duel menu. | `gDuel_bWinnerSide` | APPLIED |
| F73 | `0x80021E7C..0x80021FAC` (in `Duel_MaybeEndOfDuel` 0x800218F0) | Credit/return block after the results fade. The earlier store at 0x80021E58 sets D_8009B23A bit 0x2000 before Fade_StartOut; this block separately sets D_8009B16C bit 0x2000 at 0x80021E98, then skips credit if the winner save pointer is null. Signed `D_8009B360 >= 0` OR `gDuel_bOpponentID < 0` selects the per-save W/L path (u16 at winner ptr+0x518 and loser ptr+0x51A, normal cap 0x270F); otherwise it awards starchips (cap 0xF423F) and a card. The normal Free Duel launch stores (-1, selected grid index), so it does not select that W/L branch. Its per-duelist record is instead updated on return by FreeDuel_Init (F75/F156), with a different base, selector and cap. | (fact) | CONFIRMED |
...
| F75 | per-duelist record increment | Resolved in the Free Duel overlay inside `FreeDuel_Init`: the table base is formed at `0x801682A8` (WA in-file `0xF6D2A8`) and the increment is stored at `0x801682D4` (`0xF6D2D4`). On post-duel entry, bit `0x80` of `gFreeDuel_bReturnFlags` selects the update. The function computes `gFreeDuel_aDuelistRecords + 4 * (row * 5 + column)`, adds two bytes when outcome byte `D_8009B362` equals 1 to select losses instead of wins, increments the chosen u16, and clamps it to 999. | (resolved by F156) | CONFIRMED |
| F76 | `0x800170C8..0x8001712C` | Leaf helper: given a struct in $a0, returns (sum of s16s at +0x10,+0x12,+0x14) <<16 \| (sum at +0xE,+0x12,+0x14), each clamped to [0, 0x270F]. RESOLVED while porting to the symbols guide: its two callers at 0x8001EF1C/0x8001EF78 are the already-named `Duel_CalcBattleAttack`/`Duel_CalcBattleDefense` (they add `Duel_CalcGuardianStarBonus` 0x8001EE44 and clamp again), so this is BATTLE MATH — the packed result is DEF<<16\|ATK with the stat and terrain modifiers summed into both halves, and 0x270F here is the 9999 STAT cap, not a record cap. No relation to duel records. | `Duel_CalcCardStats` | APPLIED |
| F77 | duelist id facts | Earlier reported IDs: Simon Muran = 1, Teana = 2; DarkNite = 37 via a GameShark cross-check. The reported grid-read 8 versus later duel-ID 7 remains unexplained, but does not establish a remapping: matching FreeDuel_UpdateScreen passes the committed row*5+column to func_80024DC8, which stores it directly in gDuel_bOpponentID. The resident global strings now independently confirm DarkNite at ID 37 (0x834D) and Nitemare at ID 38 (0x834E); see the primary game description's full name-table comparison. The historical 8/7 timing discrepancy is not thereby explained. | (facts) | CONFIRMED |
...
| F82 | `0x80168C7C` (module) | Free-duel screen tick: dialog-open path (flag 0x20 of 0x801690A4) else cursor tween tick (0x80168A9C), scrollbar update (0x80168004), then input: `gInput_wPad1Held` bits 0x2000 RIGHT / 0x8000 LEFT / 0x4000 DOWN / 0x1000 UP move the pending cell; `gInput_wPad1Pressed` has 0x20 cancel (SE 8) and 0xC0 confirm: cell (0,0) opens Build Deck (mode 6), any other available cell checks the 40-slot deck at 0x801D0200 (empty slot -> SE 9 + dialog), else SE 0x30, init_800eb26c_fields_from_args(-1, row*5+col, 0x6000, 0x6000), mode byte 0x8009B26C = 3 (duel start). | `FreeDuel_UpdateScreen` | APPLIED |
...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F87 | `0x800E9EC8` (0x28 bytes) | **The screen-fade state block.** Layout proven from matched C (func_800151D8, Fade_DrawOverlay, Fade_InitOut, reset_9ec8_state_if_flag145) plus live sampling through both transitions: +0 u32 fade colour (0 black, 0x00FFFFFF white via flag 0x8009B145), +4 u8 current level, +5 u8 target level, +6 u8 flags (0x80 fade in flight, 0x01 strip-wipe mode, 0x10/0x20 colour-latch), +7 u8 step (8 in the captured menu transitions; setup also uses 12), +8 s16 head, +0xA u8[30] per-strip brightness. Live: fade-out head 384 -> -129 at 8/frame, ~48 frames; fade-in mirrors it. | `gFade_State` (struct ScreenFade) | APPLIED |
| F88 | `0x800151D8` `func_800151D8` | Mirrored strip walker, now matching resident C locally. Uses the configured step, not a fixed 8: when current < target, visits i=14..0 from middle to edges and subtracts step from the signed head accumulator; otherwise visits i=0..14 from edges to middle and adds step. Clamps between current and target and writes strips[i] AND strips[29-i]. After the pass the stored head adds step * D_8009B0D8 for increasing levels and subtracts it otherwise. Only the last clamped pair reaching target promotes current to target. See the local [band-ramp mechanics](../../fade-transition-state.md#band-ramp-mechanics); the captured visual directions remain fade-OUT edges first and fade-IN middle first. | `Fade_StepBands` | APPLIED |
| F89 | `0x80015310` `Fade_Update` | Per-frame fade update, called from Fade_DrawOverlay. Runs the strip walker while flag 0x01, or a flat level ramp otherwise; when current reaches target it clears 0x80, latches the RGB (0x8009B142..44 <- 0x8009B14A..4C) and clears/sets 0x8009B141 (overlay-enable) via func_80015CFC / func_80015D0C. | `Fade_Update` | APPLIED |
| F90 | `0x800154E4` `Fade_DrawOverlay` | Updates state first, then draws if the active flag is set OR (0x8009B141 is nonzero AND level is not 255). Band mode submits 30 320x8 boxes with input colour 255 - strips[i]; non-band mode submits one 320x240 box. If flags 0x01 and 0x02 coexist, the tail adds a 320x8 box at descriptor y=240 rather than restoring full-screen geometry. The scratchpad word is an SDK attribute: GsSortBoxFill emits rectangle command 0x62 for both 0x60000000 and 0x50000000, not command 0x60. The captured strip-only 48-frame fade had 1440 box fills, not a count guaranteed for every flag combination. See the local [draw gate and submission evidence](../../fade-transition-state.md#draw-eligibility-and-box-submission). | `Fade_DrawOverlay` | APPLIED |
| F91 | `0x800158B8` / `0x80015904` / `0x80015B00` | The fade-OUT chain both transitions use. 158B8: head=255, target=0, flags=0x80, fill all 30 strips with the current level (func_800156B8, 3-insn loop), step=12. 15904: calls 158B8 then step=8 and flags|=1 (strip mode). Local matching C also calls func_80015870 last: if D_8009B145 is nonzero it replaces the flags with 0xB0 and the step with 12, so the observed strip path requires that byte to be clear. 15B00: calls 15904 then loop_call_four_update_funcs_while_flag80 -- i.e. it BLOCKS, pumping frames until flag 0x80 drops. Circle in Free Duel: Main_RunFreeDuelMenu -> 15B00 at +2f, done at +50f, frontend reset (`Main_ResetFrontendRuntime`) at +52f, disc load (func_80013C28/LoadImage2) at +56f. X on the sub menu: Main_RunMenu (ra 0x8002D5FC) -> 15B00 at +19f after the module's 17-frame highlight animation; done +67f; load +73f. | `Fade_InitOut` / `Fade_StartOut` / `Fade_WaitOut` | APPLIED |
...
| F93 | `SD_BGMFadeOut` (0x8003FF34) | Fires in the same frame as every screen fade-out, from the mode loop: call_set_1582_1584_masked(-8, 0) on the sound state (D_8009B45C+0x1582/0x1584). Paired music fade-out at -8/frame; DOTR establishes the exact `SD_BGMFadeOut` vocabulary. | `SD_BGMFadeOut` | APPLIED |
| F94 | `0x8009B141` | Separate overlay-control byte, not an unconditional black-cover latch. func_80015CFC writes 1 and func_80015D0C writes 0. With the active flag clear, Fade_DrawOverlay requires both this byte to be nonzero and level != 255. Equal-255 completion actually leaves the byte at 1 but draws nothing; an equal-zero update entering with flags 0x80 clears it. Other flags can preserve the byte, write 0x80, or re-arm the active flag. The updater preserves an already-set high bit at entry. The earlier claim that this byte alone explains the black screen during disc loading was too broad; see the local [target-assembly evidence](../../fade-transition-state.md#draw-eligibility-and-box-submission). | `gFade_bOverlayOn` | APPLIED |
...
| F105 | `0x800371A8` `Dialog_UpdateChoice` | Per-frame tick of the choice cursor object (resident assembly): first call spawns its sprite (get_or_init_D_800EFE48_slot(id,4), draw callback Widget_UpdatePulseColour for the blink); then if gDialog_bInputState bit 0x40 (confirmed) -> gDialog_bChoice = state & 7, re-highlight, done; bit 0x80 -> cancelled; otherwise feeds the DPAD to Dialog_ReadChoiceInput. | `Dialog_UpdateChoice` | APPLIED |
| F106 | `0x8003700C` `Dialog_ReadChoiceInput` | DOWN (`gInput_wPad1Repeat` bit 0x4000) increments the selection while below gDialog_bChoiceCount, UP decrements while >= 0, bit 0x8 cycles with wrap; each move plays SE 6 and calls Dialog_HighlightChoice. Returns 0 when no relevant input. | `Dialog_ReadChoiceInput` | APPLIED |
...
| F109 | `0x8009B394` (pad word for the text system) | Matching `Input_UpdatePads` proves `gInput_wPad1Repeat` contains every newly pressed bit immediately, then held bits recur through per-button timers. The adjacent `gInput_wPad2Repeat` is the pad 2 mask. | `gInput_wPad1Repeat`, `gInput_wPad2Repeat` | APPLIED |
...
| F116 | choice = a branch inside the TEXT stream | Each of the four choice points in string 1281 is `FA` `FB 02` <Run away> FE <Keep listening> FE `FB 80` u16 u16. The two halfwords are bank offsets indexed by the answer: option 0 -> 0x15DE for all four (the shared exit: 'Wait! Stop, my prince!' / 'Drat! He's gone...'), option 1 -> the byte right after the block (0x140F, 0x147F, 0x14FC, 0x1571: the next paragraph). `Text_HandleChoiceCommand` (0x80038BF0) handles the FB byte: flag 0x80 -> advance the cursor by gDialog_bChoice*2 and hand to `Text_SetCursorOffset` (0x80038BA8), which reads the u16 and replaces the LOW 16 bits of the text cursor (bank base kept). No script opcode or game flag is touched by the answer here; the story bytecode resumes at the same opcode when the box closes, so on this screen the choice only decides how much of Simon's lecture you hear. | `Text_HandleChoiceCommand` (0x80038BF0) / `Text_SetCursorOffset` (0x80038BA8) | APPLIED |
...
### External tool: TEA-Online DUEL RESULT editor (2026-09-03, static; mirror at ~/ygofm-tools/duel-result)

Evidence here is STATIC: the site's client-side JS (three DataView patches to SLUS_014.11) cross-read against our disassembly. No live trace yet.

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F129 | `0x80021EDC` in `Duel_MaybeEndOfDuel` | End-of-duel starchip award: `lbu v1,0x3A(D_8009B1E8)` (prize, u8) + `lw v0,0x5E0(player@gp+0x2D0)` (starchips, u32), add, store, clamp 0xF423F. The nop at 0x80021EDC is the site's "additional starchips" slot (`addiu v1,v1,N`, N<=10000). Confirms player record +0x5E0 = starchips and duel object +0x3A = starchip prize, +0x3C = card-drop id. | (field names: `starchips` @+0x5E0, `starchipPrize` @+0x3A) | PENDING |
| F130 | `gText_abColorSlots` (`0x801D5708`, u8[16]) | Text colour-slot table, not a "state bar": `func_80038498` maps text bytes 0x80..0x8F through `gText_abColorSlots[b & 0xF]`; `func_8003C4E0` seeds 5 slots to 4 / active 0 / other 2. The site's "Result color" patch replaces the `sll/sra` at 0x80021C7C with `li a1,(COM<<8)\|YOU; sw a1,gText_abColorSlots` and documents the values: 0 white, 1 yellow, 2 blue, 3 green, 4 grey, 5 orange, 6 red. Result screen draws YOU/COM numbers through slots 0/1. | `gText_abColorSlots` | APPLIED |
| F131 | `gText_aBank` strings 2, 62, 63 | `FC 5A 12` is the insert-player-name control sequence (~100 uses in the bank; string 2 = "NAME:"+name). String 62 (bank +0x50E) = `f8 17 14 05 18 05 \| 24 21 35 ff \| 00 3d 2f ff` = "YOU", string 63 = "COM" (`2b 21 1e`); these are the duel-result labels. The site's "@username" patch overwrites Y O U at bank +0x514 with FC 5A 12. Charset (upper case): Y=24 O=21 U=35 C=2B M=1E N=2C A=18 E=25, ':'=48. | (facts; string ids 62/63 = duel result labels) | CONFIRMED-STATIC |

### External tool: TEA-Online Initial Deck editor (2026-09-03, static + disc; mirror at ~/ygofm-tools/initialdeck)

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F132 | WA_MRG.MRG +0xF92BD4 / +0xFBDBD4 | Starter-deck pool table, read straight off the disc (WA_MRG = LBA 10102): 7 rows x 1464 B = {u16 cards-to-draw, u16 weight[722], 18 B pad}; counts 16,16,4,1,1,1,1 (=40), every row's weights sum to 2048. The two copies are byte-identical. Cross-checking every nonzero weight against the verified card catalogue establishes rows 0-3 as monster ATK+DEF ranges 450-1050, 1100-1550, 1600-2050 and 2100-2450; row 4 contains only Dark Hole and Raigeki, row 5 the six terrain cards, and row 6 28 Equip cards. The copies sit at in-file sectors 0x1F20+0x3D4 and 0x1F76+0x3D4; the site's tool writes both. A NULL-terminated list of 7 pointers 0x8016ABD4+0x5B8*k follows and fixes the module base at 0x80168000. | `gNameEntry_aStarterSets` (0x8016ABD4), `gNameEntry_apStarterSets` (0x8016D3DC) | PENDING |
| F133 | name entry / password module layout | The 0x1EDF+0x50 (name entry) and 0x1F2F+0x56 (password) loads each carry a graphics prefix (0x41 / 0x47 sectors) then a code+data module at 0x80168000 (NameEntry_Init 0x801683EC = sub-module offset 0x3EC). The two modules are byte-identical for their first 0x7326 bytes (all code + the starter table) and differ only in later data: one shared text-entry module, two data variants, same pattern as the 7 duel-overlay copies. | (architecture fact; explains why password.txt and name_entry.txt share 0x8016D4xx) | CONFIRMED-STATIC |
| F134 | `0x8016A930` (module) | Matching starter-deck generator recovered from the disc module: per set, `n` draws of threshold `(rand()&0x7FF)+1` walked over weight[0..719] (NOT 722), one extra rand() per step; hit -> if copies[i]<3 append i+1 to `gDuel_awPlayerDeck` and `Library_UpdateCardUsedFlag(i+289)`, else redraw. So: 3-copy cap, ids 721/722 undrawable, short deck if a row sums < 2048. Same walk as Duel_SelectCardDrop. | `NameEntry_BuildStarterDeck` | APPLIED |
| F135 | `0x8016AA6C` (module) | Matching module main: clears 0x801D0000..+0x3000, printf, NameEntry_Init, then `{call_four_update_funcs; rand();} while (NameEntry_PollCompletion()==0)`, then F134, then writes `(rand_or_D_8009B09C<<8) \| XOR(gSaveData_aPlayerNameSjis[0..11])` (re-rolled until non-zero) to 0x801D0534. The function identity is applied; the save-ID field remains open. | `NameEntry_Main` (APPLIED); 0x801D0534 = `gSave_dwGameID`? | HOLD |

### External tools batch 2 (2026-09-04, static + disc; mirrors under ~/teatools)

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F136 | card art record +10976 | CORRECTION to F125: this 1408-byte region is a 40x32 8bpp thumbnail plus a 64-entry BGR555 CLUT, not a 16x88 strip. Proven two ways: mean row-to-row delta is far lower at stride 40 than stride 16 (9.4 vs 25.9 on card 5), and the trailing 128 bytes decode as 64 distinct CLUT entries with exactly one STP-set entry. A byte-identical copy of the same 1408 bytes also lives alone in WA_MRG sector (id-1), i.e. at 0x800*(id-1) -- verified for cards 1, 5, 50, 200, 722. | (facts; WA sectors 0..721 = card thumbnails) | CONFIRMED-STATIC |
| F137 | WA_MRG 0xB85000 / 0xB87800 (+0x75800 per terrain) | Duel data block tables: equip compatibility at +0x85000 (u16 key, u16 count, u16 members[count], 0 terminator: 34 groups, 4041 memberships, 8220 B) loaded to 0x8017A1D8; packed fusion table at +0x87800 (u16 index[723] then 5-byte groups of two 10-bit partner/result pairs) loaded to 0x8017C2D8. All 7 terrain copies identical. The TEA-Online tools use offsets 0x800 higher and therefore misparse a stock US file. | `gDuel_awEquipTable`, `gDuel_aFusionTable` | APPLIED |
| F138 | fusion tail overlap | The lookup consumes two pairs per 5-byte group and stops on count<=0 AFTER decrementing, so an odd count also tests the final group's second half. 263 of 293 odd-count records decode a nonzero result there, but `fusion_table_lookup` indexes by min(cardA,cardB), so a tail whose partner is below the record's own id is unreachable (245 cases); after also excluding 1 out-of-range partner and 2 shadowed by an earlier match, exactly 15 tails are reachable in play (e.g. 22+31 -> 72). Set equality confirmed against exhaustive emulation of all 261,003 unordered pairs. Note: the recomp's src/psx_fusion_db.c comment stating every trailing half decodes to 0 is factually wrong (its code is correct). | (facts) | CONFIRMED-STATIC |
| F139 | WA_MRG 0xE9B000 + 0x1800*(id-1) | Per-opponent duel data, 39 blocks of 3 sectors (disc LBA 17580+3*(id-1)): four 1460-byte weight rows (deck pool, S/A-POW drops, B/C/D drops, S/A-TEC drops) each summing to 2048 -- all 156 stock rows verified -- then a 304-byte rank table identical across blocks. Loaded by the duel bootstrap at 0x80017AB0 from sector 0x1D33 + 3*opponentId to 0x801781D8 / 0x8017878C / 0x80178D40 / 0x801792F4 / 0x801798A8. | `gDuel_awOpponentDeckPool`, `gDuel_awSaPowCardDrops`, `gDuel_awBcdCardDrops`, `gDuel_awSaTecCardDrops`, `gDuel_awRankScoreChange` | APPLIED |
| F140 | WA_MRG 0xFB9800 | Card price/password table: 723 entries of {u32 price, u32 BCD password}, indexed by card id, loaded to 0x801A8000 on the password screen. Verified Blue-Eyes 999999 / 89631139 and Mystical Elf 160 / 15025844; 24 cards carry 0xFFFFFFFE meaning no password. Purchase marks story flag 0x400+id as the one-time flag, which explains the save byte seen at F35. | `gLibrary_aPasswordCardData` (0x801A8008 = entry 1) | APPLIED |
| F141 | SLUS 0x81350 (RAM 0x80090B50) | The five credits "SECRET NO." codes as 5 x {u16 high4, u16 low4}: 86985631, 37689434, 73153736, 92886423, 53297534. `Main_RunCredits` selects entry `gSave_dwGameID % 5` (the per-save id built at name entry, F135) and shows string 0x23. | `gCredits_awSecretNumbers` | APPLIED |
| F142 | WA_MRG 0xB48000 / 0xB60800 | Game font sheet: 256x168 4bpp, 128-byte rows, low nibble = left pixel, with a 16-entry CLUT at 0xB60800. Two grids: 8x12 cells for rows 0..71 and 16x16 cells for rows 72..167. Loaded as part of one 54-sector boot block by `func_80043960` -> `File_RequestAsyncTransfer(0,0,0x1690,0x36,func_80043328)`, which also carries the UI icon sheet (0xB50000) and a 0x1000-byte CLUT block (0xB60000) that `func_80043328` uploads to VRAM at (512,248,256,8). | `gText_aFontSheet`, `Boot_LoadUiBlock` | PENDING |
| F143 | WA_MRG 0xF35000 | The 25 dialog portraits: 2432 bytes each (48x48 8bpp then a 64-entry CLUT), at 0xF35000 + 2432*(n-1), disc LBA 17888..17917. Selected by text control `F6 (0x40+n) 00`, which `func_80038EB0` turns into a 0x4C-byte face-box record in `D_800EB010` (index >= 0x41 means portrait). | `gText_aDialogPortraits` | PENDING |
| F144 | SLUS 0x12084 / 0x12060 | Two TEA-Online injection sites worth knowing as free-space precedents: the unique-drop patch hooks the `Duel_SelectCardDrop` epilogue (RAM 0x80021884) into a 128-byte stub at 0x801DB2B0 with a u16[40] table at 0x801DB330, gated on the free-duel unlock bitmap `gFreeDuel_dwUnlockedDuelists` (0x801D06F4, MSB-first); the drop-tier patch redirects RAM 0x80021860 to a stub at 0x801BFFD0. The latter collides with the recomp's psx_card_shop.c arena at 0x801BFF80. | (reclaimed-RAM precedents) | CONFIRMED-STATIC |

### Symbols promoted from the teatools cross-check (2026-09-04)

These four are applied to `tools/gen_symbols.py` (not hand-edited into symbol_addrs.txt). Each was
proven from the game's own code or data, independently of what the external tool claimed.

| # | address | what we proved | name | status |
|---|---|---|---|---|
| F145 | `0x800171A8` | NAME CORRECTION: was `select_sound_preset`, which is wrong. It is the CD-stream callback for the 235-sector (0xEB) terrain package: its caller `func_8001798C` computes `sector = gDuel_bTerrain*235 + 0x16C6`, it dispatches through the 13-entry jump table at 0x800100C0 (targets 0x800171E4..0x800174D4, already recorded as its own "tails"), the stage lengths sum to 0x75800 = 235*2048, and the function's ONLY `jal` is `LoadImage2` -- a VRAM upload, with no SPU call anywhere in its 239 instructions. | `Duel_LoadPackageStage` | APPLIED |
| F146 | `gDuel_abCardLevelAttr` (`0x801D5332`) | u8[723] card table indexed by card id with NO -1 adjustment (matching `func_800291E0`: `gDuel_adwCardStats[card_id - 1]` for stats but `gDuel_abCardLevelAttr[card_id]` for this one), split into the viewer's two nibble fields at +0x3A/+0x3B. Encoding proven statistically: the high nibble takes only values 0..7, and every card whose stats-word type is 20..23 (magic/trap/equip/ritual) has high nibble 6 or 7 while every type 0..19 monster has 0..5; the low nibble is 0 for all 101 spell/trap cards and nonzero for all 621 monsters. Card 1 decodes to attribute 0 (Light) level 8, matching Blue-Eyes White Dragon (ATK 3000 / DEF 2500 from the stats word). So the byte is `attribute << 4 | level`. | `gDuel_abCardLevelAttr` | APPLIED |
| F147 | `gText_abColorSlots` (`0x801D5708`) | u8[16] text colour-slot table, not a "state bar": `func_80038498` maps a text-stream byte with bit 0x80 set through `[byte & 0xF]`, and `func_8003C4E0` / `func_800611D0` seed entries 0..4. Corroborated independently by the TEA-Online DUEL RESULT tool, which writes the YOU/COM result-number colours into slots 0 and 1 (F130) with the palette 0 white, 1 yellow, 2 blue, 3 green, 4 grey, 5 orange, 6 red -- matching the seeded values. | `gText_abColorSlots` | APPLIED |
| F148 | `0x80090B50` | The five credits "SECRET NO." codes as 5 x {u16 high4, u16 low4}: 86985631, 37689434, 73153736, 92886423, 53297534. Referenced only from the credits loop (`Main_RunCredits`, lui/addiu pair at 0x8002DB5C/0x8002DB60), which selects entry `gSave_dwGameID % 5` (the per-save id built at name entry, F135). | `gCredits_awSecretNumbers` | APPLIED |

### FMV, card CLUT convention (2026-09-04, static + disc; from the teatools batch)

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F149 | `/DATA/MOVIE.STR` (disc LBA 202734, 17300 sectors) | The 35 MB FMV file is FIVE back-to-back copies of the same 3460-sector movie -- the first 200 sectors of each copy hash identically, and 17300 = 5 x 3460. Each copy interleaves 3028 Form1 video sectors with 432 Form2 XA audio sectors (stereo 37.8 kHz 4-bit ADPCM). Roughly 28 MB of the disc is therefore dead weight. Why it is stored five times is unexplained; nothing in the EXE references the later copies. | (fact) | CONFIRMED-STATIC |
| F150 | `0x8009151C..0x8009154F` (file `0x81D1C..0x81D4F`) | Matching `func_8005C388` establishes a 13-entry `{u16 sector_count; u16 end_frame;}` table, not two standalone globals. It sums preceding sector counts to choose the start inside `MOVIE.STR`, adds the selected count for the end sector, and passes a nonzero second field to the `StSetStream` end-frame path. Only entry 0 is populated in retail: 3080 sectors and frame 303, so the boot movie uses the first 3080 of copy 0's 3460 sectors and frames 1 through 303. | `gMovie_aStreamRanges` | APPLIED |
| F151 | card art CLUT convention | Across all 722 art CLUTs at `WA_MRG 0x169000 + 14336*(id-1) + 9792`: 675 have exactly ONE entry with the STP bit set, and 719 have entry 0 equal to 0x8000. So the stock convention is a single transparent slot at index 0, NOT STP on every opaque entry. Any tool that sets STP on all 256 entries (the TEA-Online bulk card importer does) departs from stock. Also re-verified across all 722 cards: the 40x32 thumbnail at record +10976 is byte-identical to the standalone copy at `0x800*(id-1)`, and the record's trailing pad is zero (F136 sampled 5 cards; this checked every card). | (facts) | CONFIRMED-STATIC |
| F152 | `0x800E9EA8` | RESOLVED: a pre-title-screen trace captured all seven `gFile_anLba` words after `File_SetPositionTable` initialized them from the ISO directory. The values for WA, SU, MODEL, MOVIE, SD_SE, SD_BGM, and MASTER exactly match the independently measured retail LBAs; the filename pointer after index 6 is null. `0x800E9EB4` is the MOVIE entry at index 3, not the table base. | `gFile_anLba` | APPLIED |

### Batch 3 (2026-09-04): dev paths, a disputed name, and a mod hazard

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F153 | `0x801D4FD0..0x801D5250` | HAZARD, not a game fact: the TEA-Online "Battlefield Demolisher" patch injects its routine here, which lands squarely inside `gCard_asNameSortKey` (0x801D4D8E, 722 s16 running to 0x801D5332). The 640-byte overlap destroys the sort keys of card ids 290..609, so Build Deck / trunk name sorting breaks for that range. Arithmetic re-checked here. Anyone porting that effect must relocate the payload. | (hazard note) | CONFIRMED-STATIC |
| F154 | `0x8005B64C` `select_reverb_preset` | The name is WRONG but the replacement is not yet settled, so it stays for now. The resident assembly, corroborated by the recorded nonmatching C candidate, shows a five-way preset selector dispatched through a jump table, filling RECT-shaped fields (unk4/unk6, unk30/unk32), setting destination buffers (D_8009B118, D_801DD000, D_801AF800), masking the streaming flag word D_8009B0F4 and, in preset 3, calling `LoadImage2`. Its only `jal` is `LoadImage2`; nothing touches the SPU. Its caller `File_RequestMainMenuPackage` passes the dev path string "M:/mrgSU/SU.mrg" and a 115-sector count to the CD loader. It selects where and how a streamed chunk lands, not a reverb preset. Compare F145, which was the same class of misnomer and IS settled. | (`Cd_SelectTransferPreset`? -- needs one more session) | HOLD |
| F155 | `0x800117C8`, `0x80011580`, `0x800114F8` | Konami's own build paths survive in the image as plain strings: "M:/mrgSU/SU.mrg" (twice, at 0x800114F8 and 0x800117C8) and "M:/mrgSU/model.mrg". The 0x800117C8 copy is passed as the filename argument to the CD loader by `File_RequestMainMenuPackage`. These are the only drive-letter paths in the EXE. The accepted names are applied to the two role-proven copies; the duplicate at 0x800114F8 remains address-based until its own use is traced. | `gFile_szSuMrgPath` (0x800117C8), `gFile_szModelMrgPath` (0x80011580) | APPLIED |

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F156 | `gFreeDuel_aDuelistRecords` (`0x801D071C`) | The Free Duel grid win/loss table: 40 x {u16 wins, u16 losses}, capped at 999. Slot 0 belongs to the Build Deck tile and slots 1–39 to the duelists. The free-duel disc module reads it at WA in-file `0xF6D15C`, forms the selected record address at `0xF6D2A8`, and stores the increment at `0xF6D2D4` as `gFreeDuel_aDuelistRecords + 4*(row*5+col)`. This resolves the earlier F75 search and supplies the exact write site behind F121. Extent corroborates it: 40 x 4 = 160 bytes ending at exactly 0x801D07BC, where `gDuel_awRecentCardDrops` begins, with `gFreeDuel_dwUnlockedDuelists` (0x801D06F4) just below. | `gFreeDuel_aDuelistRecords` | APPLIED |
| F157 | `0x8002CB80` `Duel_CalcGuardianStarMatchup` | The old hedge in the caller now at `src/game/duel_calc_guardian_star_bonus.c` is settled: this IS the guardian-star wheel, not a scroll helper. The TEA-Online guardian-star patch overwrites this exact 116-byte function with its own 96-byte matchup routine plus a 16 x u16 mask table, independently identifying the same behavior. The matching implementation now lives in `src/game/duel_calc_guardian_star_matchup.c`. | (name unchanged; hedge resolved) | CONFIRMED |
| F158 | password overlay `0x8016D420`, `0x8016D428` | A live password-entry trace showed `0x8016D428` advance 0, 1, 2, 3 with the highlighted digit while the object at `0x8016D420` moved its target x as `163 + 16 * index` and stayed at y 99. The eight entered bytes changed independently with Up presses. The same addresses belong to unrelated state while the name-entry overlay is resident, so these names remain password-module scoped. | `gPassword_pDigitCursorWidget`, `gPassword_nDigitIndex` | APPLIED |

### Duel-rank fusion/equip counter trace (2026-09-07)
